...
[🚀 Быстрый старт](#-быстрый-старт) • [📦 Скачать](#-скачать) • [🛠️ Разработка](#-разработка) • [📝 API](#-api)
...
| 🖥️ **Кроссплатформенность** | Windows (`.exe`, `.msi`), Linux (`AppImage`), macOS |
...
|-----|-------|-----|--------|--------|
| **Бекенд** | Rust в памяти | 5 ��инут | до 1000 записей | Автоматический |
...
│   ├── src/
│   │   ├── lib.rs             # API команды (convert_currency, get_currencies)
│   │   └── main.rs            # Entry point
